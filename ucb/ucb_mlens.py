...
for t in range(Lk, T):    
    c_mu_star_under = c*A_under.max(1)
    pi_t = solve_LP(A_over, c_mu_star_under)
    fr = get_fair_regret(A, pi_t, c_mu_star)
    tot_SWR[t] = tot_SWR[t-1] + np.sum(A_pi_star - A@pi_t)
...
# with open(f"{save_as}/A_hat_{A_choice}_{SEED}_{c}.pkl", 'wb') as f:
#     pickle.dump(A_hat, f)
